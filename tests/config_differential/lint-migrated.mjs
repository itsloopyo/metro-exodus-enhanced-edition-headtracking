// Runs core's canonical config lint over MetroExodusHeadTracking.ini and over every distinct
// file the differential test migrated (the folder it names as the argument).
//
// A migrated file keeps a player's rebound hotkeys, which the lint reports as differing from the
// fleet's defaults: that rule is for the committed file, so it is the one problem a migrated file
// may have.
...
const committed = path.join(repo, "MetroExodusHeadTracking.ini");
for (const problem of lintCanonicalConfig(fs.readFileSync(committed), options)) {
  failures.push(`MetroExodusHeadTracking.ini: ${problem}`);
}
