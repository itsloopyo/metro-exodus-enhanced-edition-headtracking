// Runs core's canonical config lint over MetroExodusHeadTracking.ini, the committed file, and
// over every distinct CameraUnlock.ini the differential test migrated (the folder it names as
// the argument).
//
// A migrated file holds a value wherever the player's differed from what default gave at that
// start, and the lint reports a global row holding a value: that rule is for the committed
// file, so it is the one problem a migrated file may have.
